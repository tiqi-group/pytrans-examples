...
from .model import CryoTrap


plt.rcParams['font.size'] = 9
electrodes_data = pathlib_path(__file__).parent / 'lionix_trap_sw.json'

with open(electrodes_data, 'r') as f:
    electrodes = json.load(f)

all_patches = []
for k, ele in electrodes.items():
    path = Path.make_compound_path(*[Path(ring) for ring in ele])
    patch = PathPatch(path)
    all_patches.append(patch)

labels_positions = [(670, 50)] + \
    [(125 * (n - 6), 180) for n in range(2, 11)] + \
    [(670, -50)] + \
    [(125 * (n - 6), -180) for n in range(2, 11)]
...
def _setup_plot_on_trap(trap: CryoTrap, vmin=-10, vmax=10, cmap='RdBu_r',
...
def plot_voltages_on_trap(trap: CryoTrap, voltages, vmin=-10, vmax=10, cmap='RdBu_r',
                          edgecolor='k', linewidth=0.5, fontsize=7, title=''):
...
def animate_waveform_on_trap(trap: CryoTrap, waveform, vmin=-10, vmax=10, cmap='RdBu_r',
                             edgecolor='k', linewidth=0.5, fontsize=7, title='',
