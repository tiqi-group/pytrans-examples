...
from .geometry.gen_trap_geometry import electrode_coords, trap_y


plt.rcParams['font.size'] = 9
electrodes_data = pathlib_path(__file__).parent / 'segtrap_electrodes.json'

with open(electrodes_data, 'r') as f:
    electrodes = json.load(f)

electrode_x = electrode_coords.mean(axis=1)
...
def _setup_plot_on_trap(trap: Segtrap, vmin=-10, vmax=10, cmap='RdBu_r',
    ...
    ax.set(
        xlim=(-3100, 3100),
        ylim=(-300, 300),
        xticks=np.arange(-3000, 3001, 1000),
